import { IsNull, type Repository, LessThanOrEqual, MoreThanOrEqual, Or } from "typeorm";
...
import type { ICreateDiscount, IUpdateDiscount, ApplicableDiscountResult } from "./discount.types";
...
export class DiscountRepository {
    ...
    async update(
        ...
        if (data.targetType || data.menuItemId !== undefined || data.serviceDefinitionId !== undefined) {
    ...
    async findApplicableForOrder(
        ...
        const orderDiscount = await this.findActiveDiscount(businessId, "ORDER", now);
        if (orderDiscount) {
            return {
                discount: orderDiscount,
                calculatedAmount: this.calculateDiscountAmount(orderDiscount, orderTotal),
            };
        }
        ...
        if (menuItemIds.length > 0) {
            ...
            for (const discount of itemDiscounts) {
                ...
                if (discount.menuItemId && menuItemIds.includes(discount.menuItemId)) {
                    ...
                    return {
                        discount,
                        calculatedAmount: this.calculateDiscountAmount(discount, orderTotal),
                    };
                }
            }
        }
    ...
    async findApplicableForService(
        ...
        if (orderDiscount) {
            return {
                discount: orderDiscount,
                calculatedAmount: this.calculateDiscountAmount(orderDiscount, servicePrice),
            };
        }
        ...
        if (serviceDiscount && this.isDiscountActive(serviceDiscount, now)) {
            return {
                ...
                calculatedAmount: this.calculateDiscountAmount(serviceDiscount, servicePrice),
            };
        }
    ...
    private calculateDiscountAmount(discount: Discount, baseAmount: number): number {
    ...
    private validateDiscountData(data: ICreateDiscount | (IUpdateDiscount & { businessId: string; targetType: DiscountTargetType })): void {
        ...
        if (data.type === "FIXED_AMOUNT" && data.value !== undefined) {
            if (data.value < 1) {
                throw new BadRequestError("Fixed amount discount must be at least 1 cent");
            }
        }

        if (data.targetType === "MENU_ITEM" && !data.menuItemId) {
            throw new BadRequestError("Menu item ID is required for MENU_ITEM discounts");
        }

        if (data.targetType === "SERVICE" && !data.serviceDefinitionId) {
            throw new BadRequestError("Service definition ID is required for SERVICE discounts");
        }

        if (data.targetType === "ORDER" && (data.menuItemId || data.serviceDefinitionId)) {
            throw new BadRequestError("ORDER discounts should not have menu item or service IDs");
        }
    }
}
