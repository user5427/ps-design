...
import { AuditActionType, AuditSecurityType, AuditType, ActionResult } from "../audit/audit-log.types";
...
export function auditActionWrapper<T extends (...args: any[]) => Promise<any>>(
    ...
    return async function (...args: Parameters<T>): Promise<Awaited<ReturnType<T>>> {
        const oldValues = await auditLogService.getEntitySnapshot(entityType, entityId);
        ...
        try {
            ...
            const newValues = await auditLogService.getEntitySnapshot(entityType, entityId);
...
export function auditSecurityWrapper<T extends (...args: any[]) => Promise<any>>(
    fn: T,
    auditLogService: AuditLogService,
...
export function auditLogWrapper<T extends (...args: any[]) => Promise<any>>(
    ...
    if (Object.values(AuditActionType).includes(auditType as AuditActionType)) {
        if (!params.entityType || !params.businessId || !params.entityId) {
            throw new Error("Business action requires entityType, businessId, and entityId");
        }
