...
declare module 'fastify' {
  ...
  interface FastifyInstance {
    parseBasicAuth: (req: FastifyRequest) => { email: string; password: string } | null
  }
}

function parseBasicAuth(req: FastifyRequest): { email: string; password: string } | null {
  ...
  fastify.addHook('onRequest', async (request, reply) => {
    ...
    try {
      ...
      if ((user as any).isPasswordResetRequired) {
        return reply.code(403).send({ error: 'Password reset required', action: '/auth/change-password' })
      }
