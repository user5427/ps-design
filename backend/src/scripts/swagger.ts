...
const routes = globSync(path.join(__dirname, "../routes/**/*.ts")).filter(
	(file) => {
		...
		const hasContent =
			...
			content
				...
				.replace(/\s/g, "").length > 0; // Remove whitespace
		return hasContent;
	},
);
...
swaggerAutogen()(outputFile, routes, doc)
	.then(() => {
		console.log(
			"Swagger documentation generated successfully at src/generated/swagger-output.json",
		);
	})
	.catch((err) => {
